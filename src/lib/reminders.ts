...
function buildReminderEmail(company: ReminderCompany, items: ReminderItem[]) {
  ...
  const html = `
    <div style="font-family:Arial,sans-serif;line-height:1.5;color:#1f2430">
      ...
      <p>Open DueSentry and review the Action Required page before the deadline.</p>
    </div>
  `;
  ...
  const text = [
    ...
    "",
    actionUrl ? `Open action items: ${actionUrl}` : "Open DueSentry and review the Action Required page before the deadline.",
  ].join("\n");
